...
import { fetchUsers } from "@/store/reducer/teacher/fetchUsersReducer";
...
const Users = () => {
  ...
  const countTeachers = () => {
    return users.filter((user) => user.teacher === true).length;
  };

  const countStudents = () => {
    return users.filter((user) => user.teacher === false).length;
  };
          ...
          <div className="flex flex-col bg-card w-full">
            ...
            <div className="flex bg-card border-b">
              <div className="w-1/3 px-6 py-4">
                {users
                  .filter((user) => user.teacher === false)
                  .map((studentUser) => (
                    <UserCard
                      key={studentUser._id}
